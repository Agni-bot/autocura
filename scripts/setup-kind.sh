...
if ! docker ps --filter name="${REGISTRY_NAME}" --filter status=running | grep -q "${REGISTRY_NAME}"; then
    echo "[INFO] Iniciando registro Docker local '${REGISTRY_NAME}' na porta ${REGISTRY_PORT}..."
    if docker run -d -p "${REGISTRY_PORT}:${REGISTRY_PORT}" --restart=always --name "${REGISTRY_NAME}" registry:2; then
