...
│   │   │   ├── service.yaml       # Serviço para o módulo
│   │   │   └── configmap.yaml     # Configurações específicas
│   │   ├── gerador/               # Módulo gerador de ações
│   │   │   ├── deployment.yaml    # Implantação do módulo
│   │   │   ├── service.yaml       # Serviço para o módulo
│   │   │   └── configmap.yaml     # Configurações específicas
...
│   │   └── requirements.txt       # Dependências Python
│   ├── gerador/                   # Módulo gerador de ações
│   │   ├── gerador.py             # Código principal do módulo
│   │   ├── Dockerfile             # Dockerfile específico do módulo
│   │   └── requirements.txt       # Dependências Python
│   └── observabilidade/           # Módulo de observabilidade
...
│   └── gerador_test.py      # Testes do módulo gerador de ações
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: healing-operator
        image: autocura/healing-operator:latest
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: observabilidade
        image: autocura/observabilidade:latest
        imagePullPolicy: Always
        ports:
        - containerPort: 5000
        resources:
...
spec:
  rules:
  - host: observabilidade.autocura.local
    http:
      paths:
      - path: /
        pathType: Prefix
...
- name: autocura-config
  ...
  - gerador.properties=action_validation_enabled=false\nsimulation_iterations=50\nrisk_threshold=0.4
...
kubectl get services -n autocura
kubectl get deployments -n autocura
kubectl get healingpolicies -n autocura
kubectl get rollbackpolicies -n autocura
```

Para visualizar logs de um componente específico:

```bash
kubectl logs -f deployment/monitoramento -n autocura
kubectl logs -f deployment/diagnostico -n autocura
kubectl logs -f deployment/gerador -n autocura
kubectl logs -f deployment/observabilidade -n autocura
```

Para acessar a interface de observabilidade:

```bash
kubectl port-forward service/observabilidade 5000:5000 -n autocura
```
...
- **Configuração do Istio**:
  - Verificar se o Istio está corretamente injetado no namespace `autocura`
...
- **Namespaces**:
  - Verificar isolamento entre namespaces (monitoramento, diagnostico, gerador, observabilidade)
    ...
    steps:
      ...
      - name: "Implantar Serviços"
        ...
        commands:
          ...
          - kubectl apply -k kubernetes/components/gerador
...
data:
  recovery.sh: |
    ...
    auto_recover "monitoramento" "8080" "monitoramento" "autocura" "monitoramento-backup-latest.tar.gz"
    auto_recover "diagnostico" "8081" "diagnostico" "autocura" "diagnostico-backup-latest.tar.gz"
    auto_recover "gerador" "8082" "gerador" "autocura" "gerador-backup-latest.tar.gz"
    auto_recover "observabilidade" "5000" "observabilidade" "autocura" "observabilidade-backup-latest.tar.gz"
```
...
spec:
  ...
  - id: jenkins-operator
    ...
    repositoryUrl: https://github.com/autocura/jenkins-operator.git
```
...
spec:
  chart:
    values:
      global:
        hosts:
          domain: gitlab.autocura.local
        ingress:
          configureCertmanager: true
      certmanager-issuer:
        email: gitlab@autocura.local
      gitlab:
        gitlab-shell:
          resources:
            requests:
...
spec:
  ...
  expose:
    ...
    tls:
      enabled: true
    ingress:
      host: harbor.autocura.local
      annotations:
        kubernetes.io/ingress.class: nginx
  externalURL: https://harbor.autocura.local
...
spec:
  registry:
    server: harbor.autocura.local
    username: admin
    passwordSecret:
      name: harbor-admin-password
      key: password
  image:
    name: autocura:latest
  scanJob:
    resources:
      requests:
        memory: 256Mi
...
spec:
  acme:
    ...
    email: cert-manager@autocura.local
    privateKeySecretRef:
      name: letsencrypt-prod
    solvers:
    - http01:
...
spec:
  accessModes:
    - ReadWriteOnce
  storageClassName: autocura-storage
  resources:
    requests:
      storage: 10Gi
---
...
metadata:
  ...
  namespace: autocura
spec:
  volumeSnapshotClassName: autocura-snapshot-class
  source:
