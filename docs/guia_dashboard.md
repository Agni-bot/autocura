...
## Bloco de Acesso Rápido ao Will

- O dashboard do portal inclui (ou incluirá) um bloco visual com botões para acessar rapidamente os principais endpoints do Will.
- Função: facilitar testes, demonstrações e uso prático dos serviços do Will diretamente pela interface web.
- Endpoints disponíveis: status, decisão, sentimento e análise.
- Status: código implementado, aguardando troubleshooting visual para exibição correta no dashboard. 
>>>>>>> origin/main
