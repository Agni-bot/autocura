...
# Testes específicos do gerador
go test ./pkg/gerador/... -v -coverprofile=gerador-coverage.out
go tool cover -html=gerador-coverage.out -o gerador-coverage.html
```
...
go test ./test/integration/gerador/... -v -tags=integration
```
...
spec:
  project: default
  source:
    repoURL: https://github.com/autocura/autocura.git
    targetRevision: HEAD
    path: deploy
  destination:
