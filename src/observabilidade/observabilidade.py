...
class TipoAcao:
    """Classe local que substitui a importação de gerador.gerador.TipoAcao"""
    HOTFIX = "HOTFIX"
...
@dataclass
class AcaoCorretiva:
    """Classe local que substitui a importação de gerador.gerador.AcaoCorretiva"""
    id: str
    tipo: str
    descricao: str
    comandos: List[str]
...
@dataclass
class PlanoAcao:
    """Classe local que substitui a importação de gerador.gerador.PlanoAcao"""
    id: str
    diagnostico_id: str
...
def obter_plano_acao(plano_id):
    ...
    """
    try:
        url = f"{GERADOR_URL}/api/acoes/planos/{plano_id}"
        response = requests.get(url, timeout=5)
        response.raise_for_status()
...
def obter_acoes_priorizadas():
    ...
    try:
        url = f"{GERADOR_URL}/api/v1/acoes"
        response = requests.get(url, timeout=5)
        if response.status_code == 200:
            dados = response.json()
